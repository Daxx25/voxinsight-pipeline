...
def render_dashboard():
    ...
    # --- DATA INGESTION UPLOAD NODE ---
    st.write("### 📂 Data Ingestion Node")
    st.markdown("Upload your structured VoC dataset to populate the executive matrices.")
    
    # Streamlit file uploader widget
    uploaded_file = st.file_uploader("Upload Historical Dataset (CSV)", type=["csv"])

    # If a file is uploaded, render the dashboard. Otherwise, show a waiting message.
    if uploaded_file is not None:
        # Load data dynamically from the uploaded file buffer in memory
        ...
        # UI state when no file has been uploaded yet
        st.info("👋 **System Standby:** Please upload a generated `mock_historical.csv` file using the uploader above to initialize the pipeline and render the dashboard.")
